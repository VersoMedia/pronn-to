 ...
 */
export const LandingBuilder = function LandingBuilder({
  title,
  description,
  addFieldLabel,
  formProp,
                  ...
                  <div className="flex items-center space-x-2">
                    {!isFieldEditableSystem && !isFieldEditableSystemButHidden && !disabled && (
                      <Switch
                        ...
                        tooltip={t("show_on_landing_page")}
                      />
                    )}
                    {isUserField && field.type !== "service" && (
                      <Button
                        color="destructive"
                    ...
                    {field.type !== "service" && (
                      <Button
                        ...
                        color="secondary"
                        onClick={() => {
                          if (field.type !== "service") editField(index, field);
                          //aqui la redirección
                        }}>
                        {t("edit")}
                      </Button>
      ...
      <DialogContent className="max-h-none p-0" data-testid="edit-field-dialog">
        <Form id="form-builder" form={fieldForm} handleSubmit={handleSubmit}>
          <div className="h-auto max-h-[85vh] overflow-auto px-8 pb-7 pt-8">
            <DialogHeader title={t("add_a_landing_element")} subtitle={t("landing_element_description")} />
            <SelectField
              ...
              options={fieldTypes.filter((f) => f.value !== "service" || !f.systemOnly)}
              label={t("input_type")}
              classNames={{
                menuList: () => "min-h-[1rem]",
              }}
